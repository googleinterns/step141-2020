...
describe('test for BioBattery class', () => {
  test('battery object throws an error if passed negative Energy', () => {
    ...
    const maxCapacity = 5;
    expect(() => new BioBattery(10, 5,  'small_battery', RESISTANCE.SMALL_BATTERY, currentpower, maxCapacity)).toThrow(
  ...
  test('battery returns less or equal to power requested', () => {
    ...
    const battery = new BioBattery(10, 5,  'small_battery', RESISTANCE.SMALL_BATTERY, currentpower, maxCapacity);
  ...
  test('battery is full when it is at capacity', () => {
    const battery = new BioBattery(10, 5,  'small_battery', RESISTANCE.SMALL_BATTERY, 20, 20);
    const expected = true;
